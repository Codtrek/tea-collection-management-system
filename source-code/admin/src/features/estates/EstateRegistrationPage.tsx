import { useState } from 'react'
import { useNavigate } from 'react-router-dom'
import { useForm, useWatch } from 'react-hook-form'
import { zodResolver } from '@hookform/resolvers/zod'
import { Upload, RouteIcon } from 'lucide-react'
import { PageHeader } from '@/components/layout/PageHeader'
import { Card } from '@/components/ui/Card'
import { Button } from '@/components/ui/Button'
import { Input } from '@/components/ui/Input'
import { Select } from '@/components/ui/Select'
import { Toggle } from '@/components/ui/Toggle'
import { MultiStepWizard } from '@/components/patterns/MultiStepWizard'
import { useToast } from '@/components/ui/Toast'
import { estateOwnerSchema, type EstateOwnerForm, BANKS, BRANCHES } from './schema'

const STEPS = [
  { id: 'owner', label: 'Owner Details', fields: ['ownerName', 'nic', 'contact', 'email'] },
  { id: 'estate', label: 'Estate Location & Route', fields: ['estateName', 'address', 'location'] },
  { id: 'bank', label: 'Bank Details', fields: ['bank', 'branch', 'account'] },
  { id: 'documents', label: 'Documents', fields: [] },
  { id: 'review', label: 'Review', fields: [] },
] as const

/* EST-02 — Administrator registers an estate owner (open item #1: consistent
   with employee registration §8.1.5). Route is auto-assigned by the system. */
export function EstateRegistrationPage() {
  const navigate = useNavigate()
  const { toast } = useToast()
  const [step, setStep] = useState(0)
  const [saving, setSaving] = useState(false)

  const {
    register,
    handleSubmit,
    trigger,
    control,
    setValue,
    formState: { errors },
  } = useForm<EstateOwnerForm>({
    resolver: zodResolver(estateOwnerSchema),
    mode: 'onBlur',
    defaultValues: { selfDelivery: false, email: '' },
  })
  const values = useWatch({ control })

  const next = async () => {
    const fields = STEPS[step].fields as readonly (keyof EstateOwnerForm)[]
    const ok = fields.length === 0 || (await trigger(fields))
    if (ok) setStep((s) => Math.min(STEPS.length - 1, s + 1))
  }

  const onSubmit = (data: EstateOwnerForm) => {
    setSaving(true)
    setTimeout(() => {
      toast('Estate owner registered')
      void data
      navigate('/estates/EST-0001')
    }, 600)
  }

  return (
    <div>
      <PageHeader
        title="Register Estate Owner"
        breadcrumb={[{ label: 'Home', to: '/dashboard' }, { label: 'Estate Owners', to: '/estates' }, { label: 'Register' }]}
      />

      <MultiStepWizard steps={STEPS.map((s) => ({ id: s.id, label: s.label }))} current={step} onStepClick={setStep}>
        <Card>
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
            {step === 0 && (
              <div className="grid gap-4 sm:grid-cols-2">
                <Input label="Owner full name" error={errors.ownerName?.message} {...register('ownerName')} />
                <Input label="NIC number" error={errors.nic?.message} {...register('nic')} />
                <Input label="Contact number" error={errors.contact?.message} {...register('contact')} />
                <Input label="Email (optional)" type="email" error={errors.email?.message} {...register('email')} />
              </div>
            )}

            {step === 1 && (
              <div className="flex flex-col gap-4">
                <div className="grid gap-4 sm:grid-cols-2">
                  <Input label="Estate name" error={errors.estateName?.message} {...register('estateName')} />
                  <Input label="Location (town)" error={errors.location?.message} {...register('location')} />
                  <div className="sm:col-span-2">
                    <Input label="Estate address / GPS" error={errors.address?.message} {...register('address')} />
                  </div>
                </div>

                {/* Route is system-assigned — read-only by architecture decision. */}
                <div className="flex items-center gap-3 rounded-[var(--radius-md)] border border-border bg-surface-sunken p-4">
                  <span className="flex size-9 items-center justify-center rounded-full bg-brand-soft text-primary">
                    <RouteIcon className="size-4" />
                  </span>
                  <div>
                    <p className="text-sm font-medium text-text">Collection route: Auto-assigned</p>
                    <p className="text-xs text-text-muted">
                      Routes are assigned by the system based on estate location — not picked at registration.
                    </p>
                  </div>
                </div>

                <div className="flex items-center justify-between rounded-[var(--radius-md)] border border-border p-4">
                  <div>
                    <p className="text-sm font-medium text-text">Self-delivery / transport exemption</p>
                    <p className="text-xs text-text-muted">
                      Owners delivering directly to the factory are exempt from the transport cost deduction.
                    </p>
                  </div>
                  <Toggle
                    checked={values.selfDelivery ?? false}
                    onChange={(v) => setValue('selfDelivery', v)}
                    label={values.selfDelivery ? 'Exempt' : 'Not exempt'}
                  />
                </div>
              </div>
            )}

            {step === 2 && (
              <div className="grid gap-4 sm:grid-cols-2">
                <Select label="Bank" placeholder="Select bank" error={errors.bank?.message} options={BANKS.map((b) => ({ value: b, label: b }))} {...register('bank')} />
                <Select label="Branch" placeholder="Select branch" error={errors.branch?.message} options={BRANCHES.map((b) => ({ value: b, label: b }))} {...register('branch')} />
                <Input
                  label="Account number"
                  hint="Masked on the profile; revealed on click (§13)."
                  error={errors.account?.message}
                  {...register('account')}
                />
              </div>
            )}

            {step === 3 && (
              <div className="grid gap-4 sm:grid-cols-2">
                {['Estate ownership / deed', 'ID proof'].map((doc) => (
                  <label
                    key={doc}
                    className="flex cursor-pointer flex-col items-center justify-center gap-2 rounded-[var(--radius-md)] border border-dashed border-border bg-surface-sunken px-4 py-8 text-center hover:bg-surface-hover"
                  >
                    <Upload className="size-6 text-text-muted" strokeWidth={1.5} />
                    <span className="text-sm font-medium text-text">{doc}</span>
                    <span className="text-xs text-text-muted">Click to upload (demo)</span>
                    <input type="file" className="hidden" />
                  </label>
                ))}
              </div>
            )}

            {step === 4 && (
              <div className="flex flex-col gap-4">
                <ReviewGroup title="Owner" rows={[['Name', values.ownerName], ['NIC', values.nic], ['Contact', values.contact], ['Email', values.email || '—']]} />
                <ReviewGroup title="Estate" rows={[['Estate', values.estateName], ['Location', values.location], ['Address', values.address], ['Route', 'Auto-assigned'], ['Self-delivery', values.selfDelivery ? 'Yes — transport exempt' : 'No']]} />
                <ReviewGroup title="Bank" rows={[['Bank', values.bank], ['Branch', values.branch], ['Account', values.account]]} />
              </div>
            )}

            <div className="flex justify-between border-t border-border pt-4">
              <Button type="button" variant="secondary" onClick={() => setStep((s) => Math.max(0, s - 1))} disabled={step === 0 || saving}>
                Back
              </Button>
              {step < STEPS.length - 1 ? (
                <Button type="button" onClick={next}>
                  Continue
                </Button>
              ) : (
                <Button type="submit" loading={saving}>
                  Register Estate Owner
                </Button>
              )}
            </div>
          </form>
        </Card>
      </MultiStepWizard>
    </div>
  )
}

function ReviewGroup({ title, rows }: { title: string; rows: Array<[string, string | undefined]> }) {
  return (
    <div className="rounded-[var(--radius-md)] border border-border p-4">
      <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-text-muted">{title}</p>
      <dl className="grid gap-x-6 gap-y-1.5 sm:grid-cols-2">
        {rows.map(([k, v]) => (
          <div key={k} className="flex justify-between gap-3 text-sm">
            <dt className="text-text-muted">{k}</dt>
            <dd className="text-text">{v || '—'}</dd>
          </div>
        ))}
      </dl>
    </div>
  )
}
