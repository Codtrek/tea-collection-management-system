export type RouteStatus = 'scheduled' | 'active' | 'completed' | 'delayed' | 'cancelled';
...
export function canStartRoute(
  ...
  requestingCollectorId: string,
): TransitionResult {
  if (routeCollectorId !== requestingCollectorId) {
    return { ok: false, error: 'Only the assigned collector can start this route' };
  }
  if (status !== 'scheduled') {
...
export function canSetDelayed(status: RouteStatus, reason: string): TransitionResult {
  if (!reason.trim()) {
...
export function canSetCancelled(status: RouteStatus, reason: string): TransitionResult {
  if (!reason.trim()) {
